...
const RECOMMENDATIONS = [
  { id: 1, type: "flight", title: "Based on your frequent searches", match: "98% Match", recommendation: "Flight to London", image: "/images/grid-5.jpg" },
...
export const RecommendedSection = () => {
  ...
  return (
    <section className="py-24 px-4 md:px-12 lg:px-24 max-w-7xl mx-auto w-full relative z-10 bg-background overflow-visible">
      ...
      {/* Background Glow — radial gradient instead of blur filter */}
      <div className="absolute top-1/2 right-0 w-[400px] h-[400px] rounded-full pointer-events-none translate-x-1/2 -translate-y-1/2" style={{ background: "radial-gradient(circle, var(--accent-secondary) 0%, transparent 70%)", opacity: 0.05 }} />
          ...
          <div key={rec.id} className="rec-item shrink-0 snap-center w-[85vw] md:w-auto group relative overflow-hidden rounded-3xl bg-surface border border-text-tertiary/10 transition-transform duration-500 hover:-translate-y-2 shadow-lg flex flex-col h-[400px]">
            ...
            <div className="relative z-10 p-6 flex flex-col h-full">
              <div className="mb-auto">
                <div className="inline-block bg-black/80 px-3.5 py-1.5 rounded-full border border-white/10 text-xs font-bold text-white mb-3 shadow-lg">
                  {rec.match}
                </div>
              ...
              <div>
                ...
                <Link
                  href="/flights"
                  className="flex items-center justify-center gap-2 w-full px-5 py-3 rounded-full font-display font-light text-sm tracking-wide text-white bg-white/20 border border-white/20 hover:bg-white hover:text-black transition-all duration-300 group/btn shadow-xl"
                >
